...
from src.models import Business, User, ConversationState, ConversationStatus, Job, Request
...
async def test_state_idle_to_confirm(test_session: AsyncMock):
    ...
    response = await service.handle_message(user_phone, "Add job John Doe Fix window 50")
    
    assert "Please confirm" in response
    
    # Verify state updated in DB
    from sqlalchemy import select
    res = await test_session.execute(select(ConversationState).where(ConversationState.phone_number == user_phone))
...
async def test_state_confirm_yes(test_session: AsyncSession):
    ...
    # Verify state reset
    res = await test_session.execute(select(ConversationState).where(ConversationState.phone_number == "123456789"))
    state = res.scalar_one()
...
async def test_undo_functionality(test_session: AsyncSession):
    ...
    job = Job(business_id=biz.id, customer_id=1, description="To be undone", status="pending")
