...
async def test_tenant_isolation(test_session: AsyncSession):
    ...
    # Create Job for A
    job_a = Job(business_id=biz_a.id, customer_id=1, description="Cleaning A", status="pending") # Mock customer ID
    test_session.add(job_a)

    # Create Job for B
    job_b = Job(business_id=biz_b.id, customer_id=2, description="Cleaning B", status="pending")
