...
@pytest.mark.asyncio
async def test_scope_enforcement_blocked(
    ...
    executor = ToolExecutor(
        test_session, biz.id, user.id, str(user.phone_number), template_service
    )
...
@pytest.mark.asyncio
async def test_scope_enforcement_allowed(
    test_session: AsyncSession, template_service: TemplateService
):
...
@pytest.mark.asyncio
async def test_scope_enforcement_default_allowed(
    test_session: AsyncSession, template_service: TemplateService
):
    ...
    res1, _ = await executor.execute(
        ManageEmployeesTool(action=EmployeeManageAction.LIST)
    )
