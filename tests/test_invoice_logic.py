...
@pytest.mark.asyncio
async def test_invoice_service_create(
...
@pytest.mark.asyncio
async def test_invoice_service_existing(
    mock_session, mock_s3_service, mock_pdf_generator
):
    service = InvoiceService(mock_session)
    job = Job(id=1)
...
@pytest.mark.asyncio
async def test_invoice_service_force_regenerate(
    mock_session, mock_s3_service, mock_pdf_generator
):
    service = InvoiceService(mock_session)
    job = Job(id=1)
...
async def test_tool_executor_send_invoice(mock_session, mock_s3_service):
    ...
    executor.invoice_service.create_invoice.assert_awaited_with(
        job, force_regenerate=False
    )
