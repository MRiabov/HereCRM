...
async def test_assign_job_conflict(async_session):
    ...
    job1 = Job(business_id=biz.id, customer_id=cust.id, description="Job 1", scheduled_at=ts, employee_id=user1.id)
    async_session.add(job1)
    await async_session.commit()
    
    # Job 2 unassigned, same time
    job2 = Job(business_id=biz.id, customer_id=cust.id, description="Job 2", scheduled_at=ts)
...
async def test_assign_job_no_conflict_different_time(async_session):
    ...
    job1 = Job(business_id=biz.id, customer_id=cust.id, description="Job 1", scheduled_at=ts1, employee_id=user1.id)
    async_session.add(job1)
    await async_session.commit()
    
    job2 = Job(business_id=biz.id, customer_id=cust.id, description="Job 2", scheduled_at=ts2)
    async_session.add(job2)
    await async_session.commit()
