...
class TestPostmarkWebhook:
    ...
    def client(self):
        ...
        with (
            patch.object(settings, "postmark_auth_user", "testuser"),
    ...
    def test_postmark_webhook_missing_from(self, client, auth_headers):
        ...
        payload = {"Subject": "Test", "TextBody": "Test body"}

        response = client.post(
            "/webhooks/postmark/inbound", json=payload, headers=auth_headers
        )
    ...
    @patch(
        "src.services.postmark_service.PostmarkService.send_email",
        new_callable=AsyncMock,
    )
    @patch("src.api.routes.check_rate_limit")
    def test_postmark_webhook_success(
        self, mock_rate_limit, mock_send_email, client, auth_headers
    ):
        ...
        mock_auth_service.get_or_create_user_by_identity.return_value = (
            mock_user,
            False,
        )
        ...
        try:
            payload = {
                ...
                "TextBody": "Hi there",
                "MessageID": "msg-123",
                "Headers": [{"Name": "In-Reply-To", "Value": "msg-old-1"}],
            }
            ...
            # Verify interactions
            mock_auth_service.get_or_create_user_by_identity.assert_called_with(
                "user@example.com"
            )
