...
async def test_autoroute_apply_execution(async_session: AsyncSession):
    ...
    emp1 = User(
        ...
        default_start_location_lat=52.52,
        default_start_location_lng=13.40,
    )
    ...
    with patch("src.tools.routing_tools.messaging_service") as mock_msg_service:
        ...
        tool_input = AutorouteTool(
            date=date.today().isoformat(), apply=True, notify=True
        )
