...
async def test_customer_pov_scenarios():
    ...
    async with AsyncClient(
        ...
        with (
            ...
            patch(
                "src.services.messaging_service.MessagingService.enqueue_message",
                new_callable=AsyncMock,
            ) as mock_enqueue,
            patch(
                "src.services.routing.ors.OpenRouteServiceAdapter.get_eta_minutes"
            ) as mock_eta,
            patch(
            ...
            mock_parser.return_value = SendStatusTool(
                query="Alice", status_type=StatusType.ON_WAY
            )
            ...
            sig_yes = hmac.new(
                secret.encode("utf-8"), payload_yes_bytes, hashlib.sha256
            ).hexdigest()
            response = await ac.post(
                "/webhook",
                content=payload_yes_bytes,
                headers={
                    "X-Hub-Signature-256": f"sha256={sig_yes}",
                    "Content-Type": "application/json",
                },
            )
            ...
            sig_cust = hmac.new(
                secret.encode("utf-8"), payload_cust_bytes, hashlib.sha256
            ).hexdigest()
            ...
            response = await ac.post(
                ...
                headers={
                    "X-Hub-Signature-256": f"sha256={sig_cust}",
                    "Content-Type": "application/json",
                },
            )
            ...
            sig_cust_yes = hmac.new(
                secret.encode("utf-8"), payload_cust_yes_bytes, hashlib.sha256
            ).hexdigest()
            response = await ac.post(
                "/webhook",
                content=payload_cust_yes_bytes,
                headers={
                    "X-Hub-Signature-256": f"sha256={sig_cust_yes}",
                    "Content-Type": "application/json",
                },
            )
            ...
            sig_inv = hmac.new(
                secret.encode("utf-8"), payload_inv_bytes, hashlib.sha256
            ).hexdigest()
            ...
            response = await ac.post(
                ...
                headers={
                    "X-Hub-Signature-256": f"sha256={sig_inv}",
                    "Content-Type": "application/json",
                },
            )
            assert "Please confirm" in response.json()["reply"]
            ...
            mock_parser.return_value = None
            with patch(
                "src.services.invoice_service.InvoiceService.create_invoice"
            ) as mock_create_inv:
            ...
            async with SessionLocal() as db:
                ...
                res = await db.execute(
                    select(Quote).where(Quote.external_token == "test_token_123")
                )
