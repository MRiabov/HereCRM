...
async def test_snapshotting_integrity(test_session: AsyncSession):
    ...
    job = Job(business_id=business.id, customer_id=customer.id, description="Old Job", value=50.0)
    line_item = LineItem(
        job=job,
...
async def test_negative_validation(test_session: AsyncSession):
    ...
    with pytest.raises(ValueError, match="Quantity cannot be negative"):
        LineItem(description="Bad Item", quantity=-1.0, unit_price=10.0, total_price=-10.0)
