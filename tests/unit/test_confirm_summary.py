...
async def test_generate_summary_quote(whatsapp_service, mock_session):
    ...
    mock_customer = Customer(
        id=1, name="Margaret", phone="1234567890", street="123 Main St"
    )
    ...
    with MagicMock() as mock_repo_class:
        with MagicMock() as mock_repo_instance:
            ...
            src.services.chat.utils.summary_generator.CustomerRepository = (
                mock_repo_class
            )
            mock_repo_class.return_value = mock_repo_instance
            mock_repo_instance.search = AsyncMock(return_value=[mock_customer])

            summary = await whatsapp_service.summary_generator.generate_summary(
                tool, user
            )
...
async def test_generate_summary_schedule(whatsapp_service, mock_session):
    # Setup
    user = User(id=1, business_id=10)
    tool = ScheduleJobTool(customer_query="John Doe", time="tomorrow at 2pm")

    mock_customer = Customer(
        id=2, name="John Doe", phone="0987654321", street="456 Oak Ave"
    )
...
async def test_generate_summary_request(whatsapp_service, mock_session):
    ...
    user = User(id=1, business_id=10)
    tool = AddRequestTool(
        customer_name="Alice", description="Call back later", time="anytime"
    )
...
async def test_generate_summary_quote_no_contact(whatsapp_service, mock_session):
    ...
    mock_customer = Customer(
        id=1, name="Margaret", phone=None, email=None, street="123 Main St"
    )
    ...
    with MagicMock() as mock_repo_class:
        with MagicMock() as mock_repo_instance:
            ...
            assert (
                "Warning: could not find a contact detail to send the quote" in summary
            )
...
async def test_generate_summary_invoice_no_contact(whatsapp_service, mock_session):
    # Setup
    user = User(id=1, business_id=10)
    tool = SendInvoiceTool(query="John Smith")

    mock_customer = Customer(
        id=4, name="John Smith", phone=None, email=None, street="789 Blvd"
    )
    ...
    with MagicMock() as mock_repo_class:
        with MagicMock() as mock_repo_instance:
            ...
            assert (
                "Warning: could not find a contact detail to send the invoice"
                in summary
            )
