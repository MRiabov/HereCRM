...
async def test_process_auth_callback_with_email(service):
    ...
    with patch("src.services.google_calendar_service.Flow") as mock_flow_class:
        ...
        with patch("src.services.google_calendar_service.build", return_value=mock_service):
            with patch("src.services.google_calendar_service.select") as mock_select:
                mock_result = MagicMock()
                mock_result.scalar_one_or_none.return_value = mock_user
                mock_db.execute.return_value = mock_result
                
                with patch("src.services.google_calendar_service.asyncio.to_thread", side_effect=lambda f, *args, **kwargs: f(*args, **kwargs)):
                    success = await service.process_auth_callback("test_code", 123, mock_db)
                    
                    assert success is True
                    assert mock_user.google_calendar_credentials["email"] == "test@example.com"
                    assert mock_user.google_calendar_sync_enabled is True
