...
@pytest.mark.asyncio
async def test_invoice_service_captures_payment_link(
    ...
    job = Job(
        id=1, business_id=1, business=business, description="Test Job", value=100.0
    )
    ...
    args, kwargs = mock_pdf_generator.generate_invoice.call_args
...
async def test_tool_executor_appends_payment_link(mock_session):
    ...
    with patch("src.tool_executor.InvoiceService") as MockInvoiceService:
        ...
        job = Job(id=50)
        invoice = Invoice(
            id=99, public_url="https://invoice.url", payment_link="https://pay.me/john"
        )
        ...
        mock_invoice_service.create_invoice = fake_create_invoice
...
async def test_pdf_generator_includes_payment_link():
    ...
    with patch("src.services.pdf_generator.Environment") as mock_env_cls:
        ...
        with patch("src.services.pdf_generator.HTML") as mock_html_cls:
            ...
            generator.generate_invoice(job, payment_link="https://pay.link")
