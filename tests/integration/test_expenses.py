...
async def test_add_expense_tool_integration(async_session):
    ...
    job = Job(
        business_id=business.id, customer_id=customer.id, description="Expense Job"
    )
    ...
    tool_no_job = AddExpenseTool(
        amount=10.0, description="Parking", category=ExpenseCategory.TRAVEL
    )
