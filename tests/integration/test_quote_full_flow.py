...
async def test_quote_full_conversational_flow(async_session):
    ...
    input_data = CreateQuoteTool(
        ...
        items=[QuoteLineItemInput(description="Cleaning", quantity=2, price=50.0)],
    )

    # Mocking external notifications (WhatsApp/PDF) to avoid actual API calls/failures
    with patch(
        "src.services.quote_service.QuoteService.send_quote", new_callable=AsyncMock
    ) as mock_send:
        ...
        )  # Or SENT depending on flow, usually Draft then sent
