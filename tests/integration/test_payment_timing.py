...
async def test_job_creation_respects_payment_timing(async_session):
    ...
    # 1. Setup Business
    business = Business(
        name="Payment Test Biz", workflow_payment_timing=PaymentTiming.PAID_LATER
    )
    ...
    job1 = await crm_service.create_job(
        customer_id=customer.id, description="Paid Later Job"
    )
    ...
    job2 = await crm_service.create_job(
        customer_id=customer.id, description="Usually Paid Job"
    )
    ...
    job3 = await crm_service.create_job(
        customer_id=customer.id, description="Always Paid Job"
    )
