...
from src.tools.employee_management import PromoteUserTool, DismissUserTool, LeaveBusinessTool
...
async def test_owner_can_promote_employee(async_session: Session):
    ...
    owner = User(name="Owner", phone_number="+100", business_id=business.id, role=UserRole.OWNER)
    employee = User(name="John Doe", phone_number="+101", business_id=business.id, role=UserRole.EMPLOYEE)
    async_session.add_all([owner, employee])
    await async_session.commit()

    # Execute
    executor = ToolExecutor(async_session, business.id, owner.id, owner.phone_number, TemplateService())
...
async def test_owner_can_dismiss_employee(async_session: Session):
    ...
    owner = User(name="Owner", phone_number="+200", business_id=business.id, role=UserRole.OWNER)
    employee = User(name="Fired Guy", phone_number="+201", business_id=business.id, role=UserRole.EMPLOYEE)
    async_session.add_all([owner, employee])
    await async_session.commit()
...
async def test_employee_can_leave_business(async_session: Session):
    ...
    employee = User(name="Quitting Guy", phone_number="+301", business_id=business.id, role=UserRole.EMPLOYEE)
    async_session.add(employee)
    await async_session.commit()

    # Execute
    executor = ToolExecutor(async_session, business.id, employee.id, employee.phone_number, TemplateService())
...
async def test_employee_cannot_promote(async_session: Session):
    ...
    pawn = User(name="Pawn", phone_number="+401", business_id=business.id, role=UserRole.EMPLOYEE)
    other = User(name="Other", phone_number="+402", business_id=business.id, role=UserRole.EMPLOYEE)
    async_session.add_all([pawn, other])
    await async_session.commit()

    # Execute
    executor = ToolExecutor(async_session, business.id, pawn.id, pawn.phone_number, TemplateService())
