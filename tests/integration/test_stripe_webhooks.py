...
async def test_stripe_webhook_checkout_completed(
    async_client, async_session, mock_stripe_signature, monkeypatch
):
    ...
    response = await async_client.post(
        "/webhooks/stripe", json=payload, headers=headers
    )
            ...
            "object": {
                ...
                "items": {
                    "data": [
                        {
                            "price": {
                                "id": "price_1SrkGWEWj96BiJElrDXGNpyz"
                            },  # Matches seat config
                            "quantity": 2,
                        },
                        {
                            "price": {
                                "id": "price_123_placeholder"
                            },  # Matches employee_management config
                            "quantity": 1,
                        },
                    ]
                },
...
async def test_stripe_webhook_invalid_signature(
    async_client, mock_stripe_signature, monkeypatch
):
    monkeypatch.setenv("STRIPE_WEBHOOK_SECRET", "whsec_test")
    import stripe

    mock_stripe_signature.side_effect = stripe.error.SignatureVerificationError(
        "Invalid sig", "sig_header", "secret"
    )
