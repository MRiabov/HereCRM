...
def mock_messaging_api():
    with (
        patch.object(
            MessagingService, "_send_whatsapp", return_value=(True, "mock_id")
        ),
...
@pytest.mark.asyncio
async def test_event_bus_integration_job_created(async_session: AsyncSession):
    """
    ...
    service = MessagingService(session_factory=session_factory)
    ...
    customer = Customer(
        business_id=business.id, name="Test Customer", phone="+1234567890"
    )
    db.add(customer)
    await db.commit()
...
@pytest.mark.asyncio
async def test_event_bus_integration_job_scheduled(async_session: AsyncSession):
    """
...
@pytest.mark.asyncio
async def test_event_bus_integration_on_my_way(async_session: AsyncSession):
    """
...
@pytest.mark.asyncio
async def test_multiple_events_concurrent_processing(async_session: AsyncSession):
    """
    ...
    for i in range(3):
        c = Customer(
            business_id=business.id, name=f"Customer {i}", phone=f"+123456789{i}"
        )
        db.add(c)
        customers.append(c)
    await db.commit()
    ...
    event1 = (
        "JOB_CREATED",
        {"job_id": 1, "customer_id": customers[0].id, "business_id": business.id},
    )
    event2 = (
        "JOB_SCHEDULED",
        {
    ...
    event3 = (
        "ON_MY_WAY",
        {"customer_id": customers[2].id, "business_id": business.id, "eta_minutes": 10},
    )
