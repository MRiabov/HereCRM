...
from src.models import Customer, Job, Request, EntityType
...
async def test_search_customers_delegation(search_service):
    ...
    params = SearchTool(query="Test", entity_type=EntityType.CUSTOMER)
...
async def test_search_jobs_delegation(search_service):
    ...
    params = SearchTool(query="Sink", entity_type=EntityType.JOB)
...
async def test_search_requests_delegation(search_service):
    ...
    params = SearchTool(query="help", entity_type=EntityType.REQUEST)
...
async def test_search_routing_lead(search_service):
    ...
    params = SearchTool(query="test", entity_type=EntityType.LEAD)
    await search_service.search(params, business_id=1)
...
async def test_search_formatting_detailed(search_service):
    ...
    c = Customer(
        name="John Doe", phone="123", street="Main St", city="Dublin", details="VIP"
    )
    search_service.customer_repo.search.return_value = [c]

    # Test Detailed
    params = SearchTool(query="John", entity_type=EntityType.CUSTOMER, detailed=True)
    result, data = await search_service.search(params, business_id=1)
...
async def test_search_truncation(search_service):
    ...
    params = SearchTool(query="all", entity_type=EntityType.CUSTOMER)
    result, data = await search_service.search(params, business_id=1)
