...
async def test_autoroute_preview_basic(async_session: AsyncSession):
    ...
    emp1 = User(
        ...
        default_start_location_lat=52.5200,
        default_start_location_lng=13.4050,
    )
    emp2 = User(
        name="Bob",
        business_id=business_id,
    ...
    # Customers
    c1 = Customer(
        name="Customer 1", business_id=business_id, latitude=52.5300, longitude=13.4150
    )
    c2 = Customer(
        name="Customer 2", business_id=business_id, latitude=52.5400, longitude=13.4250
    )
    ...
    mock_ts = MagicMock()
    mock_ts.render.return_value = (
        "Proposed Schedule: Alice assigned to Customer 1 and Customer 2"
    )
