...
async def test_pipeline_progression_scenarios(test_session):
    ...
    # Add Job 1
    with patch('src.services.pipeline_handlers.src.database.AsyncSessionLocal', mock_factory):
...
async def test_manual_update_persistence(test_session):
    ...
    c1 = Customer(name="Manual Mike", business_id=biz.id, phone="999", pipeline_stage=PipelineStage.CONTACTED)
    test_session.add(c1)
    await test_session.commit()
