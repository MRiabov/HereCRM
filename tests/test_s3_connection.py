...
def test_get_public_url():
    ...
    with (
        patch.object(settings, "s3_endpoint_url", "https://s3.backblazeb2.com"),
        ...
        service.s3_client.generate_presigned_url = MagicMock(
            return_value="https://s3.backblazeb2.com/my-bucket/test_key.pdf"
        )
...
def test_upload_file(mock_boto_client):
    ...
    with (
        ...
        mock_s3.generate_presigned_url.return_value = (
            "http://presigned.url/invoice_1.pdf"
        )

        url = service.upload_file(file_content, key)

        mock_s3.put_object.assert_called_once_with(
            Bucket="bucket", Key=key, Body=file_content, ContentType="application/pdf"
        )
