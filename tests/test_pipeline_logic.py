...
async def test_handle_job_created_first_job():
    ...
    with patch('src.services.pipeline_handlers.src.database.AsyncSessionLocal', mock_session_factory), \
         patch('src.services.pipeline_handlers.JobRepository', mock_job_repo_cls), \
         patch('src.services.pipeline_handlers.CustomerRepository', mock_customer_repo_cls):
...
async def test_handle_job_created_multiple_jobs():
    ...
    customer = Customer(id=1, business_id=1, pipeline_stage=PipelineStage.CONVERTED_ONCE)
