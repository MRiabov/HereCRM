...
async def test_availability_overlap(test_session: AsyncSession):
    ...
    assert await service.is_customer_available(customer.id, start + timedelta(minutes=30), start + timedelta(minutes=90)) == True
    
    # Check 11:00 to 12:00 (Not Available - no window covers it)
    assert await service.is_customer_available(customer.id, end, end + timedelta(hours=1)) == False
    ...
    await service.add_availability(customer.id, unavail_start, unavail_end, is_available=False)
    
    # Check 10:15 (Unavailable now because of override)
    assert await service.is_customer_available(customer.id, unavail_start + timedelta(minutes=5), unavail_start + timedelta(minutes=10)) == False
...
async def test_model_fields(test_session: AsyncSession):
    ...
    user = User(name="Worker", business_id=biz.id, default_start_location_lat=45.0, default_start_location_lng=9.0)
    test_session.add(user)
    
    job = Job(business_id=biz.id, customer_id=1, estimated_duration=120)
    test_session.add(job)
    
    svc = Service(business_id=biz.id, name="Svc", default_price=100, estimated_duration=45)
