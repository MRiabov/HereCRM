...
async def test_backup_db_success(tmp_path):
    ...
    async with SessionLocal() as session:
        ...
        with (
            patch("src.services.data_management.storage_service") as mock_storage,
            ...
            cursor = conn.cursor()
            cursor.execute(
                "SELECT name FROM businesses WHERE name='Backup Test Business'"
            )
            row = cursor.fetchone()
...
async def test_backup_endpoint_unauthorized(tmp_path):
    ...
    with patch.dict(os.environ, {"CRON_SECRET": "secret123"}):
        ...
        response = client.post(
            "/api/v1/pwa/backup/trigger", headers={"X-Cron-Secret": "wrong"}
        )
        assert response.status_code == 401
...
async def test_backup_endpoint_success(tmp_path):
    ...
    with (
        patch.dict(os.environ, {"CRON_SECRET": "secret123"}),
        patch(
            "src.services.data_management.DataManagementService.backup_db"
        ) as mock_backup,
    ):
        ...
        client = TestClient(app)
        response = client.post(
            "/api/v1/pwa/backup/trigger", headers={"X-Cron-Secret": "secret123"}
        )
