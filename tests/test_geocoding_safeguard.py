...
async def test_geocoding_safeguard_blocking():
    ...
    with patch.object(GeocodingService, 'get_coordinates', new_callable=AsyncMock) as mock_coords:
    ...
    with patch.object(GeocodingService, 'get_coordinates', new_callable=AsyncMock) as mock_coords:
        async def side_effect(text):
            if "O'Connell Street" in text:
                return 53.3498, -6.2603, {"formatted": "O'Connell Street, Dublin", "city": "Dublin"}
            if "Dublin" in text:
                return 53.3498, -6.2603, {"formatted": "Dublin, Ireland"}
            return None, None, None
