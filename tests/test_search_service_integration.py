...
async def setup_data(db_session: AsyncSession):
    ...
    c3 = Customer(
        name="Cork Lead", business_id=bus.id, latitude=CORK_LAT, longitude=CORK_LON
    )
...
async def test_search_proximity_generic_aggregation(search_service, setup_data):
    ...
    params = SearchTool(
        query="all", center_lat=51.5074, center_lon=-0.1278, radius=5000.0
    )
...
@pytest.mark.asyncio
async def test_search_with_geocoding_trigger(
    search_service, setup_data, mock_geocoding_service
):
    business_id = setup_data
    ...
    params = SearchTool(query="all", center_address="London", radius=10000.0)
