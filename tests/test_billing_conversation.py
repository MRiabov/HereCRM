...
def whatsapp_service(mock_session, mock_parser, mock_template_service):
    ...
    service = WhatsappService(
        mock_session, mock_parser, mock_template_service, billing_service=mock_billing
    )
...
async def test_billing_entry(whatsapp_service):
    ...
    with unittest.mock.patch(
        "src.services.chat.handlers.idle.ToolExecutor"
    ) as MockExecutor:
...
async def test_billing_status_check(whatsapp_service):
    ...
    with unittest.mock.patch(
        "src.services.chat.handlers.billing.ToolExecutor"
    ) as MockExecutor:
        mock_exec_instance = AsyncMock()
        MockExecutor.return_value = mock_exec_instance
...
async def test_billing_upgrade_request_seat(whatsapp_service):
    ...
    whatsapp_service.billing_handler.summary_generator.generate_summary = AsyncMock(
...
async def test_billing_upgrade_request_addon(whatsapp_service):
    ...
    response = await whatsapp_service.billing_handler.handle(
        user, state, "I want the campaign addon"
    )
