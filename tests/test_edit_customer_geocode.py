...
async def test_edit_customer_regeocodes(mock_template_service):
    ...
    async with SessionLocal() as session:
        ...
        executor = ToolExecutor(
            session, biz.id, user.id, user.phone_number, mock_template_service
        )
        ...
        add_tool = AddLeadTool(
            name="Margaret", phone="086123123", location="Bad Address"
        )
        ...
        edit_tool = EditCustomerTool(query="Margaret", location="Dublin")
        ...
        search_tool = SearchTool(query="Margaret", center_address="Dublin")
