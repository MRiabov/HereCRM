...
async def test_confirm_quote(async_session):
    ...
    # Verify Job creation
    stmt = select(Job).options(selectinload(Job.line_items)).where(Job.id == confirmed.job_id)
...
async def test_get_quote(async_session):
    ...
    service = QuoteService(async_session)
    quote = await service.create_quote(customer.id, business.id, [{"description": "Item 1", "unit_price": 50.0}])
...
async def test_get_recent_quote(async_session):
    ...
    # 1. Draft quote
    await service.create_quote(customer.id, business.id, [{"description": "Draft", "unit_price": 10.0}])
    
    # 2. Sent quote
    quote_sent = await service.create_quote(customer.id, business.id, [{"description": "Sent", "unit_price": 20.0}])
