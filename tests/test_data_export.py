...
async def test_export_customers_csv(async_session: AsyncSession):
    ...
    # Seed data
    c1 = Customer(business_id=business.id, name="Alice", phone="100", pipeline_stage=PipelineStage.CONTACTED, street="Dublin")
    c2 = Customer(business_id=business.id, name="Bob", phone="200", pipeline_stage=PipelineStage.LOST, street="Cork")
...
async def test_export_jobs_filtered(async_session: AsyncSession):
    ...
    with patch("src.services.data_management.storage_service") as mock_storage:
        ...
        assert "xlsx" in key or "excel" in content_type # Content type might be the long one
