...
async def test_llm_tracing():
    ...
    mock_message.tool_calls = [
        MagicMock(function=MagicMock(name="AddJobTool", arguments='{"customer_name": "John", "price": 50.0, "description": "test job"}'))
    ]
    mock_message.dict.return_value = {"role": "assistant", "content": mock_message.content}
    ...
    assert mock_ph.capture.call_count >= 2, "Should have at least 2 capture calls (one from wrapper, one from manual fallback)"
