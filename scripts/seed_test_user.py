...
async def seed_user():
    ...
    try:
        ...
        cursor.execute("SELECT id FROM users WHERE clerk_id = ? OR email = ?", (clerk_user_id, TEST_USER_EMAIL))
        existing_user = cursor.fetchone()
        
        if existing_user:
            print(f"User already exists in local DB with ID {existing_user[0]}")
            # Ensure clerk_id is synced if it matched by email
            cursor.execute("UPDATE users SET clerk_id = ? WHERE email = ?", (clerk_user_id, TEST_USER_EMAIL))
            conn.commit()
        else:
            ...
            if not business:
                ...
                cursor.execute("INSERT INTO businesses (name, created_at) VALUES (?, datetime('now'))", ("E2E Test Business",))
            ...
            cursor.execute(
                ...
                """,
                (
                    TEST_USER_NAME, TEST_USER_EMAIL, clerk_user_id, business_id, "OWNER",
                    "WHATSAPP", '{"confirm_by_default": false}', "UTC", 0
                )
            )
            conn.commit()
