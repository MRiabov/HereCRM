...
def main():
    parser = argparse.ArgumentParser(
        description="Batch run gemini spec-kitty tasks based on task files."
    )
    parser.add_argument(
        "--execute",
        "-x",
        action="store_true",
        help="Execute the commands. Default is dry-run.",
    )
    parser.add_argument(
        "--filter", "-f", help="Filter specs by number or name (partial match)."
    )
    parser.add_argument(
        "--force-action", choices=["implement", "review"], help="Force specific action."
    )
    ...
    for spec in specs:
        ...
        if args.filter:
            if (
                args.filter not in spec["number"]
                and args.filter.lower() not in spec["name"].lower()
            ):
                continue
    ...
    if args.execute:
        ...
        for cmd in commands:
            ...
            try:
                ...
                print(
                    "Error: 'gemini' command not found. Make sure it is installed and in your PATH."
                )
                break
