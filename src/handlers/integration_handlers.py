...
class IntegrationEventHandler:
    ...
    async def handle_job_booked(data: dict) -> None:
        ...
        async with AsyncSessionLocal() as session:
            ...
            webhook_configs = await integration_repo.get_active_by_type(
                business_id, IntegrationType.WEBHOOK
            )
            for config in webhook_configs:
                try:
                    await IntegrationEventHandler._dispatch_webhook(
                        config, job, customer
                    )
                except Exception as e:
                    logger.error(
                        f"Failed to dispatch webhook {config.id}: {e}", exc_info=True
                    )
    ...
    async def _dispatch_webhook(config: Any, job: Any, customer: Any) -> None:
        ...
        async with httpx.AsyncClient() as client:
            ...
            if response.status_code >= 400:
                logger.error(
                    f"Webhook {url} returned status {response.status_code}: {response.text}"
                )
            else:
