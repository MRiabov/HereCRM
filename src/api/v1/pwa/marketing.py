...
async def preview_audience(
    ...
    results = await service.search_service._search_customers(
        search_params, current_user.business_id, None, None
    )
    ...
    for customer in results:
        if camp_channel == CampaignChannel.EMAIL and not customer.email:
            continue
        if (
            camp_channel in [CampaignChannel.WHATSAPP, CampaignChannel.SMS]
            and not customer.phone
        ):
            continue
        valid_results.append(customer)

    return {"count": len(valid_results), "samples": [c.name for c in valid_results[:3]]}
