...
async def get_request(request_id: int, service: CRMService = Depends(get_crm_service)):
...
async def create_request(
    data: RequestCreate, service: CRMService = Depends(get_crm_service)
):
...
async def update_request(
    request_id: int, data: RequestUpdate, service: CRMService = Depends(get_crm_service)
):
    try:
...
async def delete_request(
    request_id: int, service: CRMService = Depends(get_crm_service)
):
    request = await service.request_repo.get_by_id(request_id, service.business_id)
    if not request:
        raise HTTPException(status_code=404, detail="Request not found")
