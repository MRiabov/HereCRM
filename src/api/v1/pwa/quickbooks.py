...
async def get_auth_service(session: AsyncSession = Depends(get_db)) -> QuickBooksAuthService:
    return QuickBooksAuthService(session)

async def get_accounting_service(session: AsyncSession = Depends(get_db)) -> AccountingService:
...
async def connect_quickbooks(
    ...
    if current_user.role != UserRole.OWNER:
        raise HTTPException(status_code=403, detail="Only owners can connect QuickBooks")
