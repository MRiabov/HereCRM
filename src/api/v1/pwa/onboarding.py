...
async def process_onboarding_choice(
    ...
    if payload.choice == OnboardingChoiceType.CREATE:
        ...
        if not payload.invite_code:
            raise HTTPException(
                status_code=400, detail="Invite code required to join a business"
            )

        # Use phone or email as identifier
        identifier = (
            current_user.phone_number or current_user.email or str(current_user.id)
        )

        success, message, updated_user = await invitation_service.process_join(
            identifier, code=payload.invite_code
        )
        ...
        if success:
            ...
            try:
                ...
                biz_id = (
                    updated_user.business_id
                    if updated_user
                    else current_user.business_id
                )
                role_val = (
                    updated_user.role.value if updated_user else UserRole.EMPLOYEE.value
                )

                await verify_token.clerk_client.users.update_metadata_async(
                    user_id=current_user.clerk_id,
                    public_metadata={"business_id": biz_id, "role": role_val},
                )
            except Exception as e:
                print(f"Error updating Clerk metadata: {e}")
        ...
        raise HTTPException(
            status_code=400, detail="Invalid choice. Must be 'create' or 'join'."
        )
