...
async def create_invoice(
    ...
    stmt = select(Job).where(
        Job.id == invoice_data.job_id, Job.business_id == current_user.business_id
    )
    ...
    for inv in invoices:
        ...
        response.append(
            InvoiceSchema(
                ...
                if inv.job and inv.job.customer
                else "Unknown",
            )
        )
    return response
...
async def get_invoice(
    ...
    return InvoiceSchema(
        ...
        created_at=invoice.created_at,
        public_url=invoice.public_url,
        customer_name=invoice.job.customer.name
        if invoice.job and invoice.job.customer
        else "Unknown",
    )
