...
async def get_data_activity(
    ...
    import_stmt = select(ImportJob).where(ImportJob.business_id == current_user.business_id).order_by(desc(ImportJob.created_at)).limit(10)
    export_stmt = select(ExportRequest).where(ExportRequest.business_id == current_user.business_id).order_by(desc(ExportRequest.created_at)).limit(10)
