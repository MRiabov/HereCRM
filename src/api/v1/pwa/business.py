...
async def list_employees(
    ...
    stmt = (
        ...
        .options(selectinload(User.wage_config))
...
async def update_wage_config(
    ...
    if current_user.role not in [UserRole.OWNER, UserRole.MANAGER]:
        raise HTTPException(
            status_code=403,
            detail="Only owners and managers can update wage configurations",
        )
...
async def get_business_info(
    ...
    employee_count_stmt = select(func.count(User.id)).where(
