...
async def provision_integration(
    payload: ProvisionRequest, db: AsyncSession = Depends(get_db)
):
    """
    ...
    is_valid = Signer.verify(
        payload.config_type + payload.label, payload.auth_id, settings.secret_key
    )
    if not is_valid:
    ...
    if not business_id:
        raise HTTPException(
            status_code=400, detail="business_id is required in payload"
        )
...
async def create_request(
    ...
    request = Request(
        business_id=business_id, description=full_content, status=RequestStatus.PENDING
    )
