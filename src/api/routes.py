...
class GenericWebhookPayload(BaseModel):
    identity: str = Field(
        ..., description="Phone number or email of the CRM user", max_length=100
    )
...
async def verify_postmark_auth(
    ...
    is_correct_username = hmac.compare_digest(
        credentials.username.encode("utf8"), settings.postmark_auth_user.encode("utf8")
    )
    is_correct_password = hmac.compare_digest(
        credentials.password.encode("utf8"), settings.postmark_auth_pass.encode("utf8")
    )
...
class TextGridWebhookPayload(BaseModel):
    from_number: str = Field(
        ..., alias="from", max_length=20, pattern=r"^\+?[1-9]\d{1,14}$"
    )
    to_number: str = Field(
        ..., alias="to", max_length=20, pattern=r"^\+?[1-9]\d{1,14}$"
    )
    ...
    try:
        ...
        if attachments:
            ...
            if not customer:
                ...
                clean_name = (
                    from_email.split("<")[0].strip().replace('"', "")
                    if "<" in from_email
                    else from_email.split("@")[0]
                )
    ...
    try:
        ...
        if success:
            ...
            if success_url:
                ...
                logger.info(
                    f"Google Auth successful for user {user_id}, redirecting to {safe_url}"
                )
