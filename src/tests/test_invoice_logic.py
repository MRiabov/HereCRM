...
async def test_create_invoice_success(
    session: AsyncSession, mock_s3_service, mock_pdf_generator
):
    # Setup
...
@pytest.mark.asyncio
async def test_send_invoice_tool_success(
    session: AsyncSession, mock_s3_service, mock_pdf_generator, template_service
):
    # Setup
    biz = Business(name="Test Biz")
    session.add(biz)
    ...
    user = User(
        business_id=biz.id, name="Admin", role=UserRole.OWNER, phone_number="123"
    )
    session.add(user)
    await session.flush()
...
@pytest.mark.asyncio
async def test_send_invoice_tool_no_job(
    session: AsyncSession, mock_s3_service, mock_pdf_generator, template_service
):
    # Setup
    biz = Business(name="Test Biz")
...
@pytest.mark.asyncio
async def test_existing_invoice_warning(
    session: AsyncSession, mock_s3_service, mock_pdf_generator, template_service
):
    # Setup
    biz = Business(name="Test Biz")
    ...
    existing_invoice = Invoice(
        job_id=job.id,
        s3_key="old.pdf",
        public_url="http://old.com/inv.pdf",
