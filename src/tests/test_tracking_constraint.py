...
async def test_start_job_constraint(session: AsyncSession):
    ...
    # Create two jobs
    job1 = Job(customer_id=customer.id, business_id=business.id, description="Job 1", employee_id=user.id)
    job2 = Job(customer_id=customer.id, business_id=business.id, description="Job 2", employee_id=user.id)
