...
@pytest.fixture
def mock_geocoder():
    with patch("src.services.crm_service.GeocodingService") as mock_crm_geocoder, \
         patch("src.tools.routing_tools.GeocodingService") as mock_tool_geocoder:
        
        instance = AsyncMock()
        instance.geocode.return_value = (53.3498, -6.2603, "123 Main St", "Dublin", "Ireland", "D1", "123 Main St, Dublin, Ireland, D1")
...
async def test_crm_service_update_job_geocoding(session: AsyncSession, mock_geocoder):
    ...
    mock_geocoder.geocode.return_value = (51.8985, -8.4726, "Cork St", "Cork", "Ireland", "T12", "Cork St, Cork, Ireland, T12")
...
@pytest.mark.asyncio
async def test_autoroute_executor_on_the_fly_geocoding(session: AsyncSession, mock_geocoder):
    # Setup
    biz = Business(name="Test Biz")
    session.add(biz)
    await session.commit()
    ...
    mock_routing.calculate_routes.return_value = MagicMock() # Return some solution mock
