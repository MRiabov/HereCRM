...
try:
    ...
    logging.warning(
        "pysqlcipher3 not available - QuickBooks credentials encryption disabled"
    )
...
if PYSQLCIPHER_AVAILABLE:
    ...
    if not CREDENTIALS_DB_KEY:
        logging.warning(
            "CREDENTIALS_DB_KEY environment variable is required for QuickBooks integration"
        )
    else:
        try:
    ...
    logging.info(
        "QuickBooks credentials encryption disabled - install sqlcipher system libraries"
    )
