...
from src.services.routing.base import (
...
class AutorouteToolExecutor:
    def __init__(
        ...
        if settings.openrouteservice_api_key:
            ...
            logger.info(
                "No OpenRouteService API key found, using Mock Routing Service."
            )
    ...
    async def run(self, input: AutorouteTool) -> str:
        ...
        return self.template_service.render(
            "autoroute_preview", date=target_date, routes=routes_body
        )
    ...
    async def _calculate(self, target_date: date_cls):
        ...
        # Jobs
        pending_jobs = await self.job_repo.search(
            query="all", business_id=self.business_id, status=JobStatus.PENDING
        )
        ...
        if not all_jobs:
            return (
                None,
                f"No jobs found to route for {target_date} (checked pending and scheduled).",
            )
        ...
        try:
            ...
            solution = await loop.run_in_executor(
                None, self.routing_service.calculate_routes, all_jobs, employees
            )
                ...
                for step in steps:
                    ...
                    if notify:
                        ...
                        if job.customer and job.customer.phone:
                            ...
                            logger.info(
                                f"Notification hook: Customer {job.customer.name} would be notified about job {job.id}"
                            )
    ...
    async def _format_solution_body(
        self, solution: RoutingSolution, employees: List[User]
    ) -> str:
        lines = []
        ...
        start_time_base = datetime.today().replace(
        ...
        for emp_id, steps in solution.routes.items():
            ...
            if not steps:
                ...
                for step in steps:
                    ...
                    customer_name = (
                        j.customer.name if j.customer else "Unknown Customer"
                    )
        ...
        if solution.unassigned_jobs:
            ...
            for j in solution.unassigned_jobs:
                ...
                if j.latitude is None or j.longitude is None:
                    if j.customer and (
                        j.customer.latitude is None or j.customer.longitude is None
                    ):
