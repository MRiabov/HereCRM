...
class QuoteCreationHandler:
    ...
    async def run(self, input: CreateQuoteTool) -> Tuple[str, Optional[Dict[str, Any]]]:
        ...
        customers = await self.customer_repo.search(
            input.customer_identifier, self.business_id
        )
        if not customers:
            return (
                f"Could not find customer matching '{input.customer_identifier}'",
                None,
            )
        if len(customers) > 1:
            return (
                f"Multiple customers found matching '{input.customer_identifier}'. Please be more specific.",
                None,
            )
        ...
        quote = await self.quote_service.create_quote(
            customer_id=customer.id, business_id=self.business_id, lines=lines
        )
