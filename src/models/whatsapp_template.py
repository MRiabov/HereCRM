...
class WhatsAppTemplate(Base):
    ...
    category: Mapped[WhatsAppTemplateCategory] = mapped_column(SAEnum(WhatsAppTemplateCategory))
    
    # Store the components structure as defined by Meta API (BODY, HEADER, FOOTER, BUTTONS)
    components: Mapped[Any] = mapped_column(JSON) 
    
    status: Mapped[WhatsAppTemplateStatus] = mapped_column(SAEnum(WhatsAppTemplateStatus), default=WhatsAppTemplateStatus.DRAFT)
    
    # Meta specific
    meta_template_id: Mapped[Optional[str]] = mapped_column(String, nullable=True, index=True)
