...
class ToolExecutor:
    def __init__(
        ...
        self.campaign_service = CampaignService(
            session, self.search_service, PostmarkService()
        )
    ...
    def _get_routing_service(self) -> OpenRouteServiceAdapter:
        if not self._routing_service:
            ...
            self._routing_service = OpenRouteServiceAdapter(
                api_key=settings.openrouteservice_api_key
            )
        return self._routing_service

    async def _get_user_defaults(
        self,
        ...
        ) or user.preferences.get("default_city")
        default_country = (
            business.default_country if business else None
        ) or user.preferences.get("default_country")
        ...
        return (default_city, default_country, safeguard, max_dist)
    ...
    async def execute(
        ...
        if not self.rbac_service.check_permission(user.role, tool_name):
            ...
            friendly_name = (
                tool_config.get("friendly_name", "perform this action")
                if tool_config
        ...
        if business:
            ...
            if isinstance(tool_call, SendInvoiceTool):
                if business.workflow_invoicing == InvoicingWorkflow.NEVER:
                    return (
                        "Invoicing is currently disabled in your business settings. (Owner can re-enable this by saying 'update workflow settings').",
                        None,
                    )

                # Block invoice if 'always paid on spot' (as per "Payment Tools" blocking rule)
                if (
                    business.workflow_payment_timing
                    == PaymentTiming.ALWAYS_PAID_ON_SPOT
                ):
                    return (
                        "Invoicing is disabled because your business is set to 'Always paid on spot'.",
                        None,
                    )
            ...
            if isinstance(tool_call, CreateQuoteTool):
                if business.workflow_quoting == QuotingWorkflow.NEVER:
                    return (
                        "Quoting is currently disabled in your business settings. (Owner can re-enable this by saying 'update workflow settings').",
                        None,
                    )
        ...
        if isinstance(tool_call, AddJobTool):
            ...
            return (
                f"✔ Access granted to Employee Management: {tool_call.action.value}",
                None,
            )
            ...
            asyncio.create_task(
                self.campaign_service.execute_campaign(tool_call.campaign_id)
            )
            return (
    ...
    async def _execute_add_lead(  # Renamed from _execute_add_customer
        ...
        if safeguard_enabled and default_city and not lat and tool.location:
            return (
                f"Error: The location '{tool.location}' is too far from your default city ({default_city}) or could not be found within the allowed range. Please provide a more specific address or update your default city.",
                None,
            )
    ...
    async def _execute_edit_customer(
        ...
        if tool.location:
            ...
            if safeguard_enabled and default_city and not lat:
                return (
                    f"Error: The location '{tool.location}' is too far from your default city ({default_city}) or could not be found within the allowed range.",
                    None,
                )
    ...
    async def _execute_add_job(
        ...
        lat, lon, street, city, country, postal_code, full_address = (
            None,
            None,
            None,
            None,
        ...
        if not status and not tool.time:
            settings = await self.workflow_service.get_settings(self.business_id)
            default_setting = (
                settings.get("workflow_job_creation_default")
    ...
    async def _execute_schedule_job(
        ...
        if job:
            ...
            job = await crm_service.update_job(
                ...
                description=tool.description
                or (
                    f"{job.description} (Scheduled: {tool.time})"
                    if job.description and "(Scheduled:" not in job.description
                    else job.description or f"(Scheduled: {tool.time})"
                ),
                status=JobStatus.SCHEDULED,
                value=tool.price,
    ...
    async def _execute_store_request(
        ...
        req = await crm.create_request(
            ...
            customer_details=tool.customer_details.dict()
            if tool.customer_details
            else None,
        )
    ...
    async def _execute_update_settings(
        ...
        if tool.setting_key in business_keys:
            ...
            if not user or user.role != UserRole.OWNER:
                return (
                    "Error: Only the business owner can update business-level settings.",
                    None,
                )
            ...
            new_settings = await self.workflow_service.update_settings(
                self.business_id, **updates
            )
    ...
    async def _execute_update_customer_stage(
        ...
        return (
            f"✔ Updated {customer.name}'s stage to {tool.stage.replace('_', ' ').title()}",
            {
                "action": "update",
                "entity": "customer",
                "id": customer.id,
    ...
    async def _execute_add_service(
        ...
        if existing:
            ...
            return (
                f"✔ Updated existing service *'{service_name}'* – Price: {tool.price:.2f}",
                {
                    "action": "update",
                    "entity": "service",
                    "id": existing.id,
    ...
    async def _execute_edit_service(
        ...
        updated_service = await self.service_repo.update(
            target.id, self.business_id, **updates
        )
        ...
        return (
            f"Updated service '{updated_service.name}' – Price: {updated_service.default_price:.2f}",
            {
                "action": "update",
                "entity": "service",
    ...
    async def _execute_send_invoice(
        ...
        job = await self.job_repo.get_most_recent_by_customer(
            customer.id, self.business_id
        )

        if not job:
            return (
                f"No jobs found for customer {customer.name}. Cannot generate invoice.",
                None,
            )
    ...
    async def _execute_get_billing_status(
        ...
        for addon_id in active_addons:
            if addon_id in addon_definitions:
                ...
                active_lines.append(
                    f"- {defi['name']} ({defi['price']}) - {defi['desc']}"
                )
            else:
        ...
        msg_config = self.billing_service.config.get("products", {}).get(
            "messaging", {}
        )
        if msg_config:
            upgrade_lines.append(
                f"- {msg_config.get('name', 'Messaging Package')} (€{msg_config.get('overage_rate', 0.02)}/msg overage or buy pack)"
            )
        ...
        return (
            f"{header}\n{body}",
            {"action": "query", "entity": "billing", "status": status_info},
        )
    ...
    async def _execute_request_upgrade(
        ...
        try:
            ...
            success_url = (
                f"{base_url}/billing/success?session_id={{CHECKOUT_SESSION_ID}}"
            )
    ...
    async def _execute_create_quote(
        self, tool: CreateQuoteTool
    ) -> Tuple[str, Optional[Dict[str, Any]]]:
        quote_tool = QuoteCreationHandler(
            self.quote_service,
            self.customer_repo,
            self.business_id,
    ...
    async def _execute_show_schedule(
        ...
        if tool.date:
            try:
                ...
                return self.template_service.render(
                    "error_invalid_date", date=tool.date
                ), None
    ...
    async def _execute_assign_job(
        ...
        if not employees:
            return self.template_service.render(
                "employee_not_found", name=tool.assign_to_name
            ), None

        if len(employees) > 1:
            names = ", ".join([e.name or "Unnamed" for e in employees])
            return self.template_service.render(
                "employee_ambiguous", name=tool.assign_to_name, matches=names
            ), None
    ...
    async def _execute_check_eta(
        ...
        if tool.customer_query:
            ...
            if not user or user.role != UserRole.OWNER:
                return (
                    "Error: Only business owners can query ETA for other customers.",
                    None,
                )
        ...
        if lat is None or lng is None:
            return (
                f"Technician {tech.name} is assigned but their location is currently unavailable.",
                None,
            )
        ...
        if updated_at:
            ...
            if (now - ua) > timedelta(minutes=30):
                return (
                    f"Technician {tech.name} is en route, but their location signal was lost {int((now - ua).total_seconds() / 60)} minutes ago. Please contact office.",
                    None,
                )
        ...
        if eta is None:
            return (
                f"Technician {tech.name} is en route. Unable to calculate precise ETA currently.",
                None,
            )
    ...
    async def _execute_autoroute(
        ...
        executor = AutorouteToolExecutor(
            self.session, self.business_id, self.template_service
        )
    ...
    async def _execute_update_workflow_settings(
        ...
        return "Workflow settings updated successfully.", {
    ...
    async def _execute_connect_quickbooks(
        ...
        return self.template_service.render(
            "quickbooks_connect_prompt", url=auth_url
        ), {"action": "connect_quickbooks", "url": auth_url}
    ...
    async def _execute_quickbooks_status(
        ...
        last_sync = (
            business.quickbooks_last_sync.strftime("%Y-%m-%d %H:%M")
            if business.quickbooks_last_sync
            else "Never"
        )
    ...
    async def _execute_sync_quickbooks(
        ...
        if not business.quickbooks_connected:
            return (
                "❌ QuickBooks is not connected. Say 'Connect QuickBooks' first.",
                None,
            )
        ...
        try:
            ...
            return self.template_service.render(
                "quickbooks_sync_complete", count=count
            ), {"action": "sync_quickbooks", "results": results, "count": count}
        except Exception as e:
    ...
    async def _execute_leave_business(
        ...
        if user.role == UserRole.OWNER:
            return (
                "Owners cannot leave their own business (delete the business or transfer ownership instead).",
                None,
            )
    ...
    async def _execute_connect_google_calendar(
        self, tool: ConnectGoogleCalendarTool
    ) -> Tuple[str, Optional[Dict[str, Any]]]:
        ...
        if not service.is_configured:
            return (
                "Error: Google Calendar API is not configured on the server. Please contact support.",
                None,
            )
        ...
        return self.template_service.render(
            "google_calendar_connect_prompt", url=auth_url
        ), {"action": "connect", "entity": "google_calendar"}

    async def _execute_disconnect_google_calendar(
        self, tool: DisconnectGoogleCalendarTool
    ) -> Tuple[str, Optional[Dict[str, Any]]]:
        user = await self.user_repo.get_by_id(self.user_id)
        if not user:
            return "Error: User not found.", None
        ...
        return self.template_service.render("google_calendar_disconnected"), {
            "action": "disconnect",
            "entity": "google_calendar",
        }

    async def _execute_google_calendar_status(
        self, tool: GoogleCalendarStatusTool
    ) -> Tuple[str, Optional[Dict[str, Any]]]:
        user = await self.user_repo.get_by_id(self.user_id)
        if not user:
            return "Error: User not found.", None
        ...
        connected = (
            "Yes"
            if user.google_calendar_sync_enabled and user.google_calendar_credentials
            else "No"
        )

        return (
            f"📅 *Google Calendar Status*:\n- Connected: {connected}\n- Sync Enabled: {connected}",
            {"action": "query", "entity": "google_calendar"},
        )
