...
class SearchService:
    ...
    async def search(
        ...
        if params.center_address and (
            params.center_lat is None or params.center_lon is None
        ):
            (
                lat,
                lon,
            ...
            if lat is not None and lon is not None:
                ...
                return (
                    f"Search center location '{params.center_address}' is too far from {default_city} or not found.",
                    {},
                )

        # Default radius if location present
        if (
            params.center_lat is not None and params.center_lon is not None
        ) and not params.radius:
        ...
        if params.entity_type in [EntityType.CUSTOMER, EntityType.LEAD]:
            results.extend(
                await self._search_customers(params, business_id, min_date, max_date)
            )
        elif params.entity_type == EntityType.JOB:
            results.extend(
                await self._search_jobs(params, business_id, min_date, max_date)
            )
        elif params.entity_type == EntityType.REQUEST:
            results.extend(
                await self._search_requests(params, business_id, min_date, max_date)
            )
        else:
            # Aggregate all
                ...
                serialized_results.append(
                    {
                        ...
                        if hasattr(item, "pipeline_stage") and item.pipeline_stage
                        else None,
                    }
                )
    ...
    async def _search_customers(
        self, params: SearchTool, business_id: int, min_date, max_date
    ) -> list:
