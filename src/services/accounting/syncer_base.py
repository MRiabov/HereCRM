...
class AbstractSyncer(ABC):
    ...
    async def sync(self, business_id: int, record_id: int) -> bool:
        ...
        try:
            ...
            if not record:
                logger.error(
                    f"Record not found: business_id={business_id}, record_id={record_id}"
                )
                return False
            ...
            logger.info(
                f"Successfully synced record {record_id} to QuickBooks ID {qb_id}"
            )
            return True
    ...
    async def _update_status(
        ...
        try:
            ...
            logger.error(
                f"Failed to update sync status for record {record.id}: {str(e)}"
            )
