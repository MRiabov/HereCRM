...
from src.models import Invoice, Job, Customer, LineItem, Service
...
class InvoiceSyncer(AbstractSyncer):
    ...
    async def _get_record(self, business_id: int, record_id: int) -> Optional[Invoice]:
        ...
        stmt = (
            ...
            .options(
                ...
                joinedload(Invoice.job).joinedload(Job.line_items).joinedload(LineItem.service)
            )
    ...
    def _map_record(self, invoice: Invoice) -> Dict[str, Any]:
        ...
        qb_data = {
            ...
            'TxnDate': invoice.created_at.date().isoformat() if invoice.created_at else None,
            'Line': []
        }
        ...
        for item in job.line_items:
            ...
            if item.service:
                if not item.service.quickbooks_id:
                    ...
                    raise DependencyError(f"Service {item.service.id} ({item.service.name}) is not synced to QuickBooks.")
        ...
        try:
            ...
            if 'Id' in data and data['Id']:
                try:
                    ...
                    logger.info(f"Found existing QuickBooks invoice by ID: {data['Id']}")
                except Exception as e:
                    logger.warning(f"Could not fetch invoice by ID {data['Id']}: {str(e)}")
