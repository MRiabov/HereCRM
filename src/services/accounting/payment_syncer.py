...
class PaymentSyncer(AbstractSyncer):
    ...
    async def _get_record(self, business_id: int, record_id: int) -> Optional[Payment]:
        ...
        stmt = (
            select(Payment)
            .options(
                joinedload(Payment.invoice).joinedload(Invoice.job).joinedload(Job.customer)
            )
        ...
        if payment and payment.invoice and payment.invoice.job and payment.invoice.job.business_id != business_id:
            return None
    ...
    def _map_record(self, payment: Payment) -> Dict[str, Any]:
        ...
        if not job or not job.customer:
            raise ValueError(f"Invoice {invoice.id} has no associated customer via job.")
            
        customer = job.customer
        if not customer.quickbooks_id:
            raise DependencyError(f"Customer {customer.id} is not synced to QuickBooks.")
        ...
        qb_data = {
            ...
            'TotalAmt': payment.amount,
            'TxnDate': payment.payment_date.date().isoformat() if payment.payment_date else None,
            'Line': [
                {
                    'Amount': payment.amount,
                    'LinkedTxn': [
        ...
        try:
            ...
            if 'Id' in data and data['Id']:
                try:
                    ...
                    logger.info(f"Found existing QuickBooks payment by ID: {data['Id']}")
                except Exception as e:
                    logger.warning(f"Could not fetch payment by ID {data['Id']}: {str(e)}")
