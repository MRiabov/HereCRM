...
class AutomationService:
    ...
    async def handle_quote_sent(self, data: dict):
        ...
        async with AsyncSessionLocal() as db:
            ...
            logger.info(
                f"Quote {quote_id} sent, follow-up automation active for business {business_id}"
            )
    ...
    async def handle_job_paid(self, data: dict):
        ...
        async with AsyncSessionLocal() as db:
            ...
            logger.info(
                f"Job {job_id} paid, review automation active for business {business_id}"
            )
            ...
            for business in businesses:
                ...
                for quote in quotes:
                    ...
                    stmt_check = select(MessageLog).where(
                        ...
                        MessageLog.trigger_source
                        == MessageTriggerSource.QUOTE_FOLLOWUP,
            ...
            for business in businesses:
                ...
                stmt_jobs = (
                    ...
                    .where(
                        ...
                        Job.paid == True,
                        Job.scheduled_at
                        <= cutoff,  # Using scheduled_at as proxy for completion time if begun_at not reliable
                    )
                )
                ...
                for job in jobs:
                    stmt_check = select(MessageLog).where(
                        MessageLog.business_id == business.id,
                        MessageLog.trigger_source
                        == MessageTriggerSource.REVIEW_REQUEST,
