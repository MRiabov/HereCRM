    ...
    def calculate(self, config: WageConfiguration, context: Dict[str, Any]) -> float:
        ...
        if total_seconds is not None:
            ...
            if not start_time or not end_time:
                raise KeyError(
                    "HourlyJobStrategy requires 'total_actual_duration_seconds' or 'start_time' and 'end_time' in context"
                )
...
class HourlyShiftStrategy(WageStrategy):
    def calculate(self, config: WageConfiguration, context: Dict[str, Any]) -> float:
        ...
        if not shift_start or not shift_end:
            raise KeyError(
                "HourlyShiftStrategy requires 'shift_start' and 'shift_end' in context"
            )
