...
class GoogleCalendarService:
    ...
    def get_auth_url(self, state: Optional[str] = None) -> Tuple[str, str]:
        if not self.is_configured:
            raise ValueError(
                "Google Calendar API is not configured. Missing client_id, client_secret, or redirect_uri."
            )
    ...
    async def process_auth_callback(
        self, code: str, user_id: int, db: AsyncSession
    ) -> bool:
        if not self.is_configured:
    ...
    async def _get_credentials(
        ...
        if creds.expired and creds.refresh_token:
            try:
                ...
                logger.error(
                    f"Failed to refresh Google credentials for user {user.id}: {e}"
                )
                return None
    ...
    async def _build_event_body(self, job: Job, db: AsyncSession) -> Dict[str, Any]:
        ...
        if not job.customer:
            ...
            query = (
                select(Job).where(Job.id == job.id).options(selectinload(Job.customer))
            )
            result = await db.execute(query)
        ...
        return {
            ...
            "extendedProperties": {"shared": {"herecrm_job_id": str(job.id)}},
        }

    async def create_event(
        self, job: Job, user: User, db: AsyncSession
    ) -> Optional[str]:
        ...
        if not service:
            logger.warning(
                f"Could not get calendar service for user {user.id} (likely auth failure)"
            )
            return None
        ...
        try:
            ...
            logger.error(
                f"Failed to create GCal event for job {job.id}: {e}", exc_info=True
            )
            return None
    ...
    async def update_event(self, job: Job, user: User, db: AsyncSession) -> bool:
        ...
        try:
            await asyncio.to_thread(
                service.events()
                .update(
                    calendarId="primary", eventId=job.gcal_event_id, body=event_body
                )
                .execute
            )
            ...
            if "404" in str(e):
                logger.warning(
                    f"GCal event {job.gcal_event_id} not found, clearing ID from job {job.id}"
                )
    ...
    async def delete_event(
        self, gcal_event_id: str, user: User, db: AsyncSession
    ) -> bool:
