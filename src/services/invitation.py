...
class InvitationService:
    ...
    async def create_invitation(
        self, business_id: int, inviter_id: int, identifier: str
    ) -> Invitation:
        """
        ...
        existing_invites = await self.invitation_repo.get_pending_by_identifier(
            identifier
        )
        invitation = None
    ...
    async def process_join(
        self, identifier: str, code: Optional[str] = None
        ...
        if existing_user:
            ...
            }  # Reset or keep? Let's keep existing props but ensuring basics
        else:
            # Create new user
            user = User(
