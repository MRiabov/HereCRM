...
class TimeTrackingService:
    ...
    async def check_out(
        self, user_id: int
    ) -> Tuple[User, datetime, datetime, List[Job]]:
        user = await self.session.get(User, user_id)
        if not user:
            raise ValueError(f"User {user_id} not found")
    ...
    async def start_job(self, job_id: int, user_id: int) -> Job:
        ...
        if active_job:
            raise ValueError(
                f"Cannot start job: You already have an active job in progress (Job #{active_job.id}). Please pause it or finish it first."
            )
