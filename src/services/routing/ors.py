    ...
    def calculate_routes(
        ...
        if not jobs:
            return RoutingSolution(
                {e.id: [] for e in employees}, [], {"status": "no_jobs"}
            )
        if not employees:
        ...
        valid_employees = [
            e
            for e in employees
            if e.default_start_location_lat is not None
            and e.default_start_location_lng is not None
        ]

        if not valid_jobs:
            return RoutingSolution(
                {e.id: [] for e in employees}, jobs, {"status": "no_valid_jobs"}
            )

        if not valid_employees:
            return RoutingSolution(
                {}, jobs, {"status": "no_valid_employees_with_location"}
            )
        ...
        try:
            ...
            raise RoutingException(
                f"Failed to parse OpenRouteService response: {str(e)}"
            )
    ...
    def build_payload(self, jobs: List[Job], employees: List[User]) -> Dict[str, Any]:
        ...
        for job in jobs:
            payload_job = {
                ...
                "service": (job.estimated_duration or 60)
        ...
        return {"jobs": ors_jobs, "vehicles": vehicles}

    def parse_response(
        self, response: Dict[str, Any], original_jobs: List[Job], employees: List[User]
    ) -> RoutingSolution:
        """
        ...
        return RoutingSolution(
            routes=routes, unassigned_jobs=unassigned_jobs, metrics=metrics
        )
